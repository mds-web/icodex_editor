...
FileContentStorage,
 ...
 getAllProjects, 
 addProject, 
 addPreferences } from './module/FileContentStorage.js';
...
const APP_SETTINGS_NAME = "Preferences-settings";
    ...
    class Input {
        constructor(placeholder = "") {
            this.input = document.createElement("input"), this.input.spellcheck = !1, this.input.autocomplete = "off", 
            this.input.placeholder = placeholder, this.input.type = "text", this.input.maxLength = 50, 
            this.input.value = "", this.input.className = "form-input";
        }
        set value(val) {
    ...
    class Button {
        constructor(className = "btn", icon = "") {
            this.button = document.createElement("button"), this.button.className = className, 
    ...
    function createElement(tag, attrs = {}) {
        ...
        for (const key in attrs) key in el ? el[key] = attrs[key] : el.setAttribute(key, attrs[key]);
        return el;
    }
    ...
    class ixScrollManager {
        constructor(target, mode = "both") {
            this.target = target, this.mode = mode, this.handlers = {
        ...
        lock() {
            this.target && ("both" !== this.mode && "desktop" !== this.mode || this.target.addEventListener("wheel", this.handlers.wheel, {
                passive: !1
            }), "both" !== this.mode && "mobile" !== this.mode || this.target.addEventListener("touchmove", this.handlers.touchmove, {
                passive: !1
            }));
        }
        unlock() {
            this.target && (this.target.removeEventListener("wheel", this.handlers.wheel), this.target.removeEventListener("touchmove", this.handlers.touchmove));
        }
    }
...
updateLanguage() {
	...
	#initCustomDialog() {
		...
		const buttonNegative = new Button("btn button-negative", i18n.t("buttonCancel"));
		buttonNegative.button.dataset.lang = "buttonCancel"; // <- untuk auto translate

		const buttonPositive = new Button("btn button-positive", i18n.t("buttonCreate"));
  ...
  this._group = group;
    ...
    this._group.forEach(group => {
        ...
        group.option.forEach(opt => {
            ...
            if (opt.icon.startsWith('bi') || opt.icon.startsWith('codicon')) opt_icon.className = opt.icon;
    ...
    class CustomDropdown extends Dropdown {
        constructor(container, handle) {
            super(), this.handle = handle, this.container = container, this.dropDown = document.createElement("div"), 
            this.dropDown.className = "dropdown", this.borderOverlay = document.createElement("div"), 
            this.borderOverlay.className = "ix-drop-border", this.scrollManager = new ixScrollManager(this.dropDown, "mobile");
        }
        #render(render = !1) {
            !0 === render || 1 === render ? (this.container.innerHTML = "", this.dropDown.innerHTML = "", 
            this.dropDown.appendChild(super.fragmentRender(!0)), DropdownPosition(this.container, this.handle, !0), 
            this.container.appendChild(this.dropDown)) : (DropdownPosition(this.container, this.handle, !1), 
    ...
    return this;
        ...
        #icodexPlaceDropdown() {
            const rect = this.dropDown.getBoundingClientRect(), windowHeight = window.innerHeight;
            rect.y < windowHeight - rect.height ? this.dropdown.style.top = rect.top + "px" : this.dropDown.style.top = `-${rect.height}px`;
        }
    } 
    
    const DROPDOWN_CONTAINERS = document.createElement("div");
    DROPDOWN_CONTAINERS.className = "dropdown-container", document.body.insertAdjacentElement("afterbegin", DROPDOWN_CONTAINERS);
Icodex.settingsPage ??= new class {
		...
		if (optgroup) {
			for (const [groupKey, opts] of Object.entries(options)) {
				...
				opts.forEach(([val, label]) => {
					const opt = new Option(label, val, false, this.settings[settingKey] === val);
	...
	render() {
		const displayItems = [
			this.createSetting("language", this.createSelect("language", [
				["en", "English"], ["id", "Indonesia"], ["es", "Español"], ["fr", "Français"],
				["de", "Deutsch"], ["zh", "中文"], ["ja", "日本語"], ["ko", "한국어"],
				["ru", "Русский"], ["pt", "Português"], ["hi", "हिन्दी (Hindi)"]
			], "language")),

			this.createSetting("theme", this.createSelect("theme-select", {
				"light-themes": [
					["light", "Light"], ["one_light", "One Light"], ["chrome", "Chrome"], ["xcode", "XCode"]
				],
				"dark-themes": [
					["dark", "Dark"], ["dracula", "Dracula"], ["dracula_x", "Dracula-X"],
					["mirage", "Mirage"], ["monokai", "Monokai"], ["one_dark", "One Dark"], ["github_dark", "Github Dark"]
				]
			}, "theme", true)),

			this.createSetting("background", this.createSelect("background", [
				["Dark", "Dark"], ["Light", "Light"], ["System", "System"]
			], "background")),

			this.createSetting("textSize", this.createSelect("text-size", [
		...
		const editorItems = [
			...
			this.createSetting("cursorStyle", this.createSelect("cursor-style", [
				["ace", "Default"], ["smooth", "No Blink"], ["slim", "Slim"]
			], "cursorStyle")),
			this.createSetting("editorFontSize", this.createSelect("editor-font-size", [
				["12px", "12px"], ["14px", "14px"], ["16px", "16px"], ["18px", "18px"]
			], "editorFontSize")),
			this.createSetting("editorFontWeight", this.createSelect("editor-font-weight", [
				["400", "Thin"], ["normal", "Normal"], ["bold", "Bold"]
			], "editorFontWeight"))
		];
...
class SearchBox {
  constructor() {
    ...
    this.icon.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-search-icon lucide-search"><path d="m21 21-4.34-4.34"/><circle cx="11" cy="11" r="8"/></svg>';
...
constructor(data) {
    if (Array.isArray(data)) {
        const valid = data.every(item => typeof item === 'object' && item !== null && !Array.isArray(item));
        if (!valid) throw new Error("TreeView error: array harus berisi object saja");
        this.data = data;
...
TreeView.prototype.getNodeByPath = function getNodeByPath(path) {
        	...
        	for (const part of parts) {
        		if (currentNode = currentNodes.find(n => n.name === part), !currentNode) return null;
...
TreeView.prototype.searchTree = function searchTree(keyword) {
  ...
  const allLabels = this.mainParentTree.querySelectorAll(".folder-name, .file-name");

  // Reset semua tampilan dan hapus highlight
  folderItems.forEach(el => el.style.display = "");
  fileItems.forEach(el => el.style.display = "");
  const directoryContents = this.mainParentTree.querySelectorAll(".directory-content");
  ...
  allLabels.forEach(label => {
    ...
    if (text.includes(normalized)) {
      ...
      label.innerHTML = rawText.replace(regex, `<span class="highlight">$1</span>`);
  ...
  directoryContents.forEach(dir => {
    const hasVisibleChild = dir.querySelector(".folder-item:not([style*='display: none']), .file-item:not([style*='display: none'])");
    if (!hasVisibleChild) {
...
TreeView.prototype.createErrorMessageElement = function createErrorMessageElement(text) {
...
TreeView.prototype.renderTree = function renderTree() {
  ...
  this.mainParentTree.insertAdjacentElement("afterbegin", this.containerLoading);
...
TreeView.prototype.createNode = function createNode(node, level = 0) {
    return node ? "directory" === node.type ? this.createDirectoryNode(node, level) : "file" === node.type ? this.createFileNode(node, level) : null : null;
        }
...
fileType.className = "validation-icon-type", fileType.setAttribute("data-validation-type", "unknown"),
...
const searchBoxEl = searchBox.element
 ...
 searchBox.close.addEventListener('click', () => {
	...
	const extTypeMap = { html: "html", js: "js", css: "css", md: "md", json: "json", txt: "txt", }; // lanjutkan sesuai aslinya
	...
	const validateFileName = (value) => {
		...
		const isDuplicate = (root.children || []).some(child => child.type === "file" && child.name === fileName);
	...
	inputNewFile.addEventListener("input", () => validateFileName(inputNewFile.value));
      ...
      $btn.addEventListener("click", () => {
        ...
        const validate = (val) => {
          ...
          for (let i = 0; i < parts.length; i++) {
            ...
            const found = cur.children.find(c => c.name === part && c.type === "directory");
            if (!found) break;
            cur = found;
        ...
        $inputFolder.addEventListener("input", () => validate($inputFolder.value));
...
TreeView.prototype.createDirectoryNode = function createDirectoryNode(node, level = 0) {
...
_toggle_three_dots.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="ic-three-dots" viewBox="0 0 16 16"><path d="M3 9.5a1.5 1.5 0 1 1 0-3 1.5 1.5 0 0 1 0 3m5 0a1.5 1.5 0 1 1 0-3 1.5 1.5 0 0 1 0 3m5 0a1.5 1.5 0 1 1 0-3 1.5 1.5 0 0 1 0 3"/></svg>';
...
content.classList.contains('open') ?  chevron.style.rotate = '90deg' : chevron.style.rotate = '0deg';
});
...
TreeView.prototype.setupDirectoryDropdown = function setupDirectoryDropdown(node, dropdownInstance) {
let self = this;
...
var tagErrorFolder = this.createErrorMessageElement("path tidak boleh kosong");
var tagErrorRenameFolder = this.createErrorMessageElement("path tidak boleh kosong");
...
newFile.show(true);
	...
	const extTypeMap = {
		...
		json: "json", xml: "xml", yml: "yml", yaml: "yaml", md: "md", txt: "txt",
		ini: "ini", env: "env", cfg: "cfg", conf: "conf", log: "log", gitignore: "gitignore",
		gitattributes: "gitattributes", editorconfig: "editorconfig", py: "py", rb: "rb", php: "php",
		java: "java", c: "c", h: "h", cpp: "cpp", hpp: "hpp", cs: "cs", go: "go", rs: "rs", swift: "swift",
		sh: "sh", bash: "bash", zsh: "zsh", pl: "pl", lua: "lua",
		csv: "csv", tsv: "tsv", xls: "xls", xlsx: "xlsx", doc: "doc", docx: "docx", pdf: "pdf", rtf: "rtf",
		jar: "jar", png: "png", jpg: "jpg", jpeg: "jpeg", gif: "gif", svg: "svg", webp: "webp",
		ico: "ico", bmp: "bmp", mp3: "mp3", wav: "wav", mp4: "mp4", webm: "webm", ogg: "ogg", mov: "mov",
		ttf: "ttf", otf: "otf", woff: "woff", woff2: "woff2",
		zip: "zip", rar: "rar", tar: "tar", gz: "gz", tgz: "tgz", "7z": "7z", sql: "sql", db: "db",
		lock: "lock", bak: "bak", map: "map", license: "license",
		vue: "vue", svelte: "svelte", astro: "astro", dockerfile: "dockerfile", toml: "toml",
		makefile: "makefile", gradle: "gradle", babelrc: "babelrc", eslintrc: "eslintrc"
	};
	...
	const validateFileName = (value) => {
		...
		const isDuplicate = (node.children || []).some(child => child.type === "file" && child.name === fileNameOnly);
	...
	inputFileEl.addEventListener("input", () => validateFileName(inputFileEl.value));
...
else if ("copy-path-folder" === clicked.id) { IcodexEditor.plugins.clipboard.copy(node.path); 
}
	...
	const parentNode = self.findParentNode(rootProject, node);
	if (!parentNode) return void console.error("Parent folder tidak ditemukan.");

	if (!window.confirm(`Yakin ingin menghapus folder "${node.name}" beserta semua isinya?`)) return;
    ...
    const rootNode = node;
     window.IcodexEditor.plugins.ZipPlugin.exportTreeToZip(rootNode, `${rootNode.name}.zip`);
                } 
    })
...
const _language_icon = document.createElement('span');
  ...
  const typeMap = {
    html: 'html', css: 'css', js: 'js', ts: 'ts', json: 'json', md: 'md', txt: 'txt',
    png: 'png', jpg: 'jpg',jsx: 'jsx', svg: 'svg', mp4: 'mp4', py: 'py', java: 'java', vue: 'vue',
    yml: 'yml', yaml: 'yaml', env: 'env', lock: 'lock', toml: 'toml', scss: 'scss', htm: 'html'
  };
...
TreeView.prototype.setupFileDropdown = function setupFileDropdown(node, anchor) {
            const self = this, _toggle_three_dots = document.createElement("span");
            _toggle_three_dots.className = "_toggle_three_dots-horizontal", _toggle_three_dots.tabIndex = 0, 
            _toggle_three_dots.role = "button", _toggle_three_dots.ariaLabel = "Open Dropdown", 
            _toggle_three_dots.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg"  fill="currentColor" class="ic-three-dots" viewBox="0 0 16 16"><path d="M3 9.5a1.5 1.5 0 1 1 0-3 1.5 1.5 0 0 1 0 3m5 0a1.5 1.5 0 1 1 0-3 1.5 1.5 0 0 1 0 3m5 0a1.5 1.5 0 1 1 0-3 1.5 1.5 0 0 1 0 3"/></svg>', 
...
this.mainParentTree.addEventListener("scroll", e => {
 ...
 const hideDropdown = () => {
    TreeView.currentOpenDropdown && (TreeView.currentOpenDropdown.open(!1), TreeView.currentOpenDropdown = null)
}
...
icodeXFileDropdown.addEventListener("optclick", e => {
    ...
    if ("rename-file" === clicked.id) {
        ...
        const rootProject = self.findRootProject(self.data, node)
        if (!rootProject) return void console.error("Root project tidak ditemukan saat rename file.")
        const parentNode = self.findParentNode(rootProject, node)
        if ((parentNode?.children || []).some(child => child !== node && "file" === child.type && child.name === trimmedName)) {
            return void alert(`File dengan nama "${trimmedName}" sudah ada di folder ini.`)
        }
        const oldPath = node.path
        node.name = trimmedName
        const newPath = `${parentNode?.path || "/"}/${trimmedName}`.replace(/\/+/g, "/")
        node.path = newPath
        self.refreshAndSave(rootProject)
    ...
    const rootProject = self.findRootProject(self.data, node)
    if (!rootProject) return void console.error("Root project tidak ditemukan.")
    const parentNode = self.findParentNode(rootProject, node)
    if (!parentNode || !Array.isArray(parentNode.children)) {
        return void console.error("Parent node tidak ditemukan atau tidak valid.")
    }
    const index = parentNode.children.findIndex(child => child === node)
    if (-1 !== index) {
        parentNode.children.splice(index, 1)
...
TreeView.prototype.findParentNode = function findParentNode(currentNode, targetNode) {
...
TreeView.prototype.getOpenFolderPaths = function getOpenFolderPaths() {
            const openFolders = [], nodes = document.querySelectorAll(".folder-item.open");
...
TreeView.prototype.reopenFolders = function reopenFolders(paths) {
            document.querySelectorAll(".directory").forEach(dir => {
                const nameEl = dir.querySelector(".folder-item"), contentEl = dir.querySelector(".directory-content");
                if (!nameEl || !contentEl) return;
                const fullPath = this.getFullPath(nameEl);
                paths.includes(fullPath) && (nameEl.classList.add("open"), contentEl.classList.add("open"));
            });
        }
 
 TreeView.prototype._replaceTree = function _replaceTree(newTreeElement) {
            const oldTree = document.querySelector(".tree-view");
            oldTree && oldTree.parentNode && oldTree.parentNode.replaceChild(newTreeElement, oldTree);
        }

TreeView.prototype.handleAddItem = async function handleAddItem(type, name, node, content = "") {
    if (!name.trim()) return;
    ...
    if (type === "file" && isNested) {
        ...
        return void await this.handleAddItem("file", fileName, parentFolder, content)
    }
    ...
    const isDuplicate = node.children.some(child =>
...
TreeView.prototype._findPathInProjectTree = function _findPathInProjectTree(root, targetNode, currentPath = []) {
            if (root === targetNode) return currentPath.length ? [ ...currentPath, root.name ].join("/") : "/";
...
TreeView.prototype.findRootProject = function findRootProject(projectList, targetNode) {
  for (const project of projectList) {
   if (this.containsNode(project, targetNode))
    return project;
        }
            return null;
        }
...
TreeView.prototype.containsNode = function containsNode(current, target) {
            return current === target || !!current.children && current.children.some(child => this.containsNode(child, target));
        }

TreeView.prototype.createNestedDirectory =    async function createNestedDirectory(path, rootNode) {
            const parts = path.split("/").filter(Boolean);
            let current = rootNode;
            for (const part of parts) {
                current.children || (current.children = []);
                let existing = current.children.find(child => child.name === part && "directory" === child.type);
                if (!existing) {
                    ...
                    existing = {
                        ...
                        path: fullPath,
                        children: []
                    }, current.children.push(existing), this.sortNodeChildren(current), console.log(`[TreeView] Folder bertingkat dibuat: ${fullPath}`);
                }
                current = existing;
            }
            const rootProject = this.findRootProject(this.data, rootNode);
            return rootProject && (await updateProject(rootProject), this.refreshTree()), current;
        }

TreeView.prototype.sortNodeChildren = function sortNodeChildren(node) {
            node.children && node.children.sort((a, b) => a.type === b.type ? a.name.localeCompare(b.name) : "directory" === a.type ? -1 : 1);
        }

TreeView.prototype.addItemFromOutside = function    addItemFromOutside(type, name, node, content = "") {
            return this.handleAddItem(type, name, node, content);
        }


TreeView.prototype.instanceMethodCount = function instanceMethodCount() {
	return Object.getOwnPropertyNames(TreeView.prototype)
    ...
    window.IcodexEditor = window.IcodexEditor || {}, IcodexEditor.plugins = IcodexEditor.plugins || {};
    const CRC32 = (() => {
        ...
        return {
            from: function(input) {
                let data = input instanceof Uint8Array ? input : (new TextEncoder).encode(String(input)), crc = 4294967295;
                for (let i = 0; i < data.length; i++) crc = table[255 & (crc ^ data[i])] ^ crc >>> 8;
                return ~crc >>> 0;
            }
        };
    })();
    class ZipEntry {
        constructor(name, data, offset) {
            this.name = name, this.data = data, this.offset = offset, this.size = data.length, 
            this.crc = CRC32.from(data), this.date = new Date;
        }
        toLocalHeader() {
            const nameBytes = (new TextEncoder).encode(this.name), header = new Uint8Array(30 + nameBytes.length), view = new DataView(header.buffer), {dosTime: dosTime, dosDate: dosDate} = this.#getDosDateTime();
            return view.setUint32(0, 67324752, !0), view.setUint16(4, 20, !0), view.setUint16(6, 0, !0), 
            view.setUint16(8, 0, !0), view.setUint16(10, dosTime, !0), view.setUint16(12, dosDate, !0), 
            view.setUint32(14, this.crc, !0), view.setUint32(18, this.size, !0), view.setUint32(22, this.size, !0), 
            view.setUint16(26, nameBytes.length, !0), view.setUint16(28, 0, !0), header.set(nameBytes, 30), 
            header;
        }
        toCentralDirectory() {
            const nameBytes = (new TextEncoder).encode(this.name), header = new Uint8Array(46 + nameBytes.length), view = new DataView(header.buffer), {dosTime: dosTime, dosDate: dosDate} = this.#getDosDateTime();
            return view.setUint32(0, 33639248, !0), view.setUint16(4, 20, !0), view.setUint16(6, 20, !0), 
            view.setUint16(8, 0, !0), view.setUint16(10, 0, !0), view.setUint16(12, dosTime, !0), 
            view.setUint16(14, dosDate, !0), view.setUint32(16, this.crc, !0), view.setUint32(20, this.size, !0), 
            view.setUint32(24, this.size, !0), view.setUint16(28, nameBytes.length, !0), view.setUint16(30, 0, !0), 
            view.setUint16(32, 0, !0), view.setUint16(34, 0, !0), view.setUint16(36, 0, !0), 
            view.setUint32(38, 0, !0), view.setUint32(42, this.offset, !0), header.set(nameBytes, 46), 
            header;
        }
        #getDosDateTime() {
            const d = this.date;
            return {
                dosTime: d.getHours() << 11 | d.getMinutes() << 5 | d.getSeconds() / 2,
    ...
    class MiniZip {
        ...
        file(path, content = "") {
            const data = content instanceof Uint8Array ? content : (new TextEncoder).encode(String(content)), entry = new ZipEntry(path, data, this.offset), local = entry.toLocalHeader(), full = new Uint8Array(local.length + data.length);
            full.set(local, 0), full.set(data, local.length), this.entries.push({
                entry: entry,
                data: full
        ...
        generateBlob() {
            ...
            for (const {entry: entry, data: data} of this.entries) buffers.push(data), central.push(entry.toCentralDirectory());
            const centralStart = this.offset;
            for (const d of central) buffers.push(d), this.offset += d.length;
            const eocd = new Uint8Array(22), view = new DataView(eocd.buffer);
            return view.setUint32(0, 101010256, !0), view.setUint16(8, central.length, !0), 
            view.setUint16(10, central.length, !0), view.setUint32(12, this.offset - centralStart, !0), 
            view.setUint32(16, centralStart, !0), buffers.push(eocd), new Blob(buffers, {
    ...
    IcodexEditor.plugins.ZipPlugin = {
        ...
        exportTreeToZip(rootNode, zipName = "project.zip") {
            ...
            !function addToZip(node, zipFolder) {
                if ("file" === node.type) zipFolder.file(node.name, node.content || ""); else if ("directory" === node.type && node.children) {
                    const folder = zipFolder.folder(node.name);
                    node.children.forEach(child => addToZip(child, folder));
                }
            }(rootNode, zip);
            const blob = zip.generateBlob(), a = document.createElement("a");
            a.href = URL.createObjectURL(blob), a.download = zipName, a.click(), URL.revokeObjectURL(a.href);
        }
    };
...
class TreeTabView {
	...
	async refresh() {
		...
		let currentId = Icodex.currentProjectId || localStorage.getItem("currentProjectId");
		if (!currentId) {
			this.#container.innerHTML = "<p style='padding:1rem;'>Pilih proyek terlebih dahulu.</p>";
			return;
		}
		...
		if (!activeProject) {
			...
			this.#container.innerHTML = "<p style='padding:1rem;'>Proyek tidak ditemukan.</p>";
			return;
		}
...
class TabDrawer extends DocumentFragment {
 ...
 class ProjectHome {
	...
	constructor() {
		...
		this.#btnAdd.addEventListener('click', () => this.#projectDialog.show(true));
...
`
	...
	async loadAllProjectsFromDB() {
		...
		for (const proj of projects) {
			const exists = this.#ulList.querySelector(`[data-project-id="${proj.id}"]`);
...
function renderPageToContent(pageName) {
	...
	if (elDom && !elDom.isConnected) {
		...
		if (pageName === "workplace" && typeof el.loadAllProjectsFromDB === "function") {
...
#setActiveTab(tabId) {
	...
	buttons.forEach(item => {
		if (item.tabbuttons) {
			item.tabbuttons.classList.toggle("active", item.tabbuttons.id === tabId);
		}
	});
}
...
const OVERLAY_DRAWER = document.createElement("div");
    OVERLAY_DRAWER.id = "overlay-drawer", document.body.insertAdjacentElement("afterbegin", OVERLAY_DRAWER);
